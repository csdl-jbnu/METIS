...
module Basepair
    ...
    private Basepair_Chimera_Cylinder_v02
...
subroutine Basepair_Discretize(prob, geom, mesh)
    ...
    !call Basepair_Chimera_Cylinder_v02(prob, geom, mesh, "cylinder_prior")
    ...
    call Basepair_Chimera_Cylinder_v02(prob, geom, mesh, "cylinder_final")
...
! Write the cylindrial model - version 2
! Version 2 - deleted the gray color for the mitered edge
subroutine Basepair_Chimera_Cylinder_v02(prob, geom, mesh, mode)
    type(ProbType), intent(in) :: prob
    type(GeomType), intent(in) :: geom
    type(MeshType), intent(in) :: mesh
    ...
    if(mode == "cylinder_prior") path = trim(prob.path_work)//"/"//trim(prob.name_file)//"_"
    if(mode == "cylinder_final") path = trim(prob.path_work)//"/"//trim(prob.name_file)//"_"
    open(unit = 502, file = trim(path)//trim("cylinder_model")//".bild", form = "formatted")
    ...
    write(502, "(a, 3f9.4)"), ".color steel blue"
    do i = 1, geom.n_croL
    ...
    close(unit = 502)
end subroutine Basepair_Chimera_Cylinder_v02
...
subroutine Basepair_Write_Edge_Length(prob, geom)
    ...
    if(p_ATHENA == .false.) path = trim(prob.path_work)
    if(p_ATHENA == .false.) open(unit = 505, file = trim(path)//"/TXT_Edge_Length.txt", form = "formatted")
    if(p_ATHENA == .false.) write(505, "(4a15)"), "Multi line", "Init line", "Section ID", "Edge length"
    ...
    do i = 1, size(geom.croL)
        ...
        if(p_ATHENA == .false.) write(505, "(4i15)"), i, iniL, sec, n_bp
    ...
    if(p_ATHENA == .false.) write(505, "(a)")
    if(p_ATHENA == .false.) write(505, "(2a15)"), "Edge length", "# of multi lines"

    ! Write edge length and count
    max_edge = num_bp(1)
    min_edge = num_bp(1)
    do i = 1, n_numbp
        if(p_ATHENA == .false.) write(505, "(2i15)"), num_bp(i), num_count(i)
    ...
    if(p_ATHENA == .false.) close(unit = 505)
