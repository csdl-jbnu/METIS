...
subroutine Input_Print_Prob_METIS
    ...
    write(0, "(a)"), "       5. Star,                  6. Curved Beam"
...
subroutine Input_Set_Prob(prob, geom)
    ...
    if(prob.sel_prob /= 0) call Input_Sel_Prob_METIS(prob, geom)
...
subroutine Input_Sel_Prob_METIS(prob, geom)
    ...
    select case (prob.sel_prob)
        ...
        case ( 5); call Exam_METIS_Star           (prob, geom)
...
subroutine Input_Chimera_Init_Geom(prob, geom)
    ...
    path = trim(prob.path_work)//"/"//trim(prob.name_file)
    open(unit = 102, file = trim(path)//"_target_geometry.bild", form = "formatted")
    ...
    open(unit = 102, file = trim(path)//"_target_geometry.dat", form = "formatted")
