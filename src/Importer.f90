...
subroutine Importer_PLY(prob, geom)
    ...
    character(10)  :: string
    integer :: i, status, npoint, ioerr, num
    logical :: here
    ...
    open(unit = 1001, file = trim(path), form = "formatted", action='READ')

    do
        read(1001, '(a)', iostat = status), ctemp

        ! Negative value, means the end-of-file (EOF) mark was read
        if(status < 0) exit

        if(index(ctemp, "element")) then

            if(index(ctemp, "element vertex ")) then

                string = ctemp(16:16+9)
                read(string, '(i10)'), num
                geom.n_iniP = num
                allocate(geom.iniP(geom.n_iniP))
            else if(index(ctemp, "element face ")) then

                string = ctemp(14:14+9)
                read(string, '(i10)'), num
                geom.n_face = num
                ...
                write(p_redir, "(a)")
                write(p_redir, "(a)"), " +=== error ========================================+"
                write(p_redir, "(a)"), " | This PLY is not supported                        |"
                write(p_redir, "(a)"), " +==================================================+"
                write(p_redir, "(a)")
                stop
            end if
...
subroutine Importer_GEO(prob, geom)
    ...
    if(geom.n_face == 0) then
        ...
        !DEC$ ELSE
        results = systemqq("python /home/polyhedra/"//trim("METIS")//&
    ...
    if(geom.n_face == 1) then
        ...
        if(p_mesh >= 0.0d0 .and. p_mesh < 1.0) then
            ...
            results = systemqq("/usr/local/matlab/R2017a/bin/matlab wait -nodisplay -nosplash -nodesktop -noawt -r "//&
                '"addpath /home/polyhedra/'//trim("METIS")//'/build/tools/DistMesh/src; addpath /home/polyhedra/'//&
                trim("METIS")//'/build/tools/DistMesh; DistMesh_linux('&
                //"'"//trim(fullname)//"',"//trim(Dble2Str(p_mesh))//')"')
            ...
            ! Check file
            inquire(file = trim(prob.path_input)//trim(fullname), exist = here)
            if(here == .false.) then
                write(p_redir, "(a)")
                write(p_redir, "(a)"), " +=== error ========================================+"
